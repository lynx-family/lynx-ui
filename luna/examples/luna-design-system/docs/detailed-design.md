...
These are implementation stages, distinct from the Parameters, Composition,
and Recipes layers in the [Working Design Brief](./design-doc.md#a-minimal-model).
The v1 implementation carries one narrow parameter slice, semantic colors,
through every stage. Later slices can reuse the pipeline while introducing
layer-specific normalized structures and contracts.
